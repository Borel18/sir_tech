...
if (!empty($_POST) && !empty($_POST['email'])) {
  $query = "SELECT * FROM livreur WHERE email = ? AND confirmed_at IS NOT NULL";
  // on cree une requete
...
if ($user) {
  ...
  $query= "UPDATE livreur SET reset_token = ?,reset_at = NOW() WHERE id=?";
  // on cree une requete
  $req = $pdo->prepare($query);
  ...
  http://localhost/sir_tech/livreur/reset.php?id=$user->id&token=$reset_token";
