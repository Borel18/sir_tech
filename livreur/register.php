...
//pseudo
    if(empty($_POST['username']) || !preg_match("#^[a-zA-Z0-9_]+$#",$_POST['username'])){
        $errors['username']= "vote pseudo n'est pas valable";
         ...
         $query = "SELECT * FROM livreur WHERE username = ?";
...
//email
    if(empty($_POST['email']) || !filter_var($_POST['email'],FILTER_VALIDATE_EMAIL)){
        ...
        $query = "SELECT * FROM livreur WHERE email = ?";
        $req = $pdo->prepare($query);
    ...
    if (empty($errors)) {
        $query = "INSERT INTO livreur (username,email,password,confirmation_token) VALUES(?,?,?,?)";
        $req = $pdo->prepare($query);
        ...
        http://localhost/sir_tech/livreur/confirm.php?id=$userId&token=$token";
