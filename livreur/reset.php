...
$query = "SELECT * FROM livreur WHERE id = ? AND reset_token= ? AND reset_at > DATE_SUB(NOW(), INTERVAL 130 MINUTE) AND confirmed_at IS NOT NULL ";
...
if($user){
  if (!empty($_POST)) {
    if(!empty($_POST['password']) || $_POST['password'] == $_POST['password_confirm'] ){
      ...
      $query= "UPDATE livreur SET password = ?,reset_token = NULL,reset_at = NULL WHERE id = ?"; 
      $pdo -> prepare($query)-> execute([$password,$userId]);
      $_SESSION['flash']['sucess'] = "votre mot de passe a bien ete reinitialisé";
      $_SESSION['auth']= $user;
      header('location:C:\xampp\htdocs\sir_tech\compte admin\admin.php');
      exit();
    }else{
